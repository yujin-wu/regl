...
const { proxyHeader } = require('./in-machine-proxy');
...
module.exports.createInterpreterEnvironment = function () {
  ...
  function setValue(path, value) {
    const rawValue = machineToRaw(value);
  ...
  function call(path, args) {
    const rawArgs = args.map(arg => machineToRaw(arg));
    const fn = getRawValue(path);
    if (typeof fn !== 'function') {
      throw new Error('Not a function');
    }
    const raw = fn(...rawArgs);
    if (typeof raw === 'object' || typeof raw === 'function') {
      const newPath = ['_retobj' + objCnt++];
  ...
  function compile(linkedNames, linkedValues, code, procNames) {
    ...
    for (var i = 0; i < linkedNames.length; i++) {
      ...
      const value = linkedValues[i];
      if (typeof value === 'object') {
        const key = `_global_${name}`;
        data[key] = value;
